...
public class View {
	...
	public void Display(Graphics2D g, Calendar c) {
		...
		g.drawImage(leftImage, 35 + 326, 11, 75 + 326, 51, 0, 0, leftImage.getWidth(null), leftImage.getHeight(null), null);
		g.drawImage(leftImage, GUI.instance.getWidth() - 405, 11, GUI.instance.getWidth() - 365, 51, leftImage.getWidth(null), 0, 0, leftImage.getHeight(null), null);
			...
			for(int day = 0; day < 7; ++day) {
				...
				if(events != null && !events.isEmpty()) {
					for(int i = 0; i < events.size(); i++) {
						...
						String eventTitle = chopTitle(CalMathAbs.ToHM(events.get(i).getTimeStart())+"-" + CalMathAbs.ToHM(events.get(i).getTimeEnd()) + " " + events.get(i).getTitle(), 24);
		...
		if(isDisplayingEvents() || isDisplayingEvent()) {
			...
			if(eventGridY > 2) {
				//Show above...
				eventBox = new Point(width * eventGridX - (eventGridX <= 1 ? -5 + (eventGridX)* width : boxWidth / 2) - (eventGridX >= 5 ? boxWidth/2  - (6 - eventGridX) * width+ 5 : 0), height * (eventGridY+1) - currentBoxHeight - 55);
			...
			g.drawLine(popupPoly.xpoints[0], popupPoly.ypoints[0] + 55, popupPoly.xpoints[0] + width + boxWidth, popupPoly.ypoints[0] + 55);
			...
			String dateString = today.get(Calendar.DATE) + " " + months[today.get(Calendar.MONTH)] + ", " + today.get(Calendar.YEAR);
			Point datePoint = null;
			
			if(!isDisplayingEvent()) {
				//set up how the thing is rendered.
				
				datePoint = View.centerString(dateString, new Rectangle(popupPoly.xpoints[0] + boxWidth/2 , popupPoly.ypoints[0] + 15, width, 30), editMetric);
			...
			g.drawImage(leftImage1, datePoint.x - 50, datePoint.y - 26,datePoint.x - 10, datePoint.y + 40 - 26, 0, 0, leftImage1.getWidth(null), leftImage1.getHeight(null), null);
			
			g.drawImage(leftImage1, 2 * popupPoly.xpoints[0] + boxWidth + width - datePoint.x + 10,datePoint.y - 20 - 6, 2 * popupPoly.xpoints[0] + boxWidth+ width - datePoint.x + 10 + 40, datePoint.y + 20 - 6, leftImage1.getWidth(null), 0, 0, leftImage1.getHeight(null), null);
			
			g.drawImage(xImage, popupPoly.xpoints[0] + 10, popupPoly.ypoints[0] + 10, popupPoly.xpoints[0] + xImage.getWidth(null) + 10, popupPoly.ypoints[0] + xImage.getHeight(null) + 10, 0, 0, xImage.getWidth(null), xImage.getHeight(null), null);
		}
	...
	public void renderEvents(Graphics2D g, int topLeftX, int topLeftY) {
		...
		Point addPoint = View.centerString(addNew, new Rectangle(topLeftX + 30, topLeftY + 70, boxWidth + width - 60, 45), bodyMetric);
		...
		for(int i = 0; i < events.size(); i++) {
			...
			// Render events!
			String renderString =chopTitle(CalMathAbs.ToHM(events.get(i + scrollBar).getTimeStart())+"-" + CalMathAbs.ToHM(events.get(i + scrollBar).getTimeEnd()) + " " + events.get(i + scrollBar).getTitle(),29); 
			Point renderEvent = View.centerString(renderString, new Rectangle(topLeftX + 30, topLeftY + 70 + 65 + 55 * i, boxWidth + width - 60 - (renderBar ? 40 : 0), 45), bodyMetric);
		...
		//Draw arrows
		if(renderBar) {
			g.drawImage(upImage, topLeftX + boxWidth + width - upImage.getWidth(null) - 10, topLeftY + 70 + 65 , topLeftX + boxWidth + width - 10, topLeftY + 70 + 65 + upImage.getHeight(null), 0, upImage.getHeight(null), upImage.getWidth(null), 0, null);
			g.drawImage(upImage, topLeftX + boxWidth + width - upImage.getWidth(null) - 10, topLeftY + 70 + 65 + 55*3, topLeftX + boxWidth + width - 10, topLeftY + 70 + 65 + upImage.getHeight(null) + 55*3, 0, 0, upImage.getWidth(null), upImage.getHeight(null), null);
		}
	...
	public void renderEvent(Graphics2D g, int topLeftX, int topLeftY) {
		...
		Point titlePoint = View.centerString(dispTitle, new Rectangle(topLeftX + 12, topLeftY + 10 + 55, boxWidth + width - 24, 45), editMetric);
		...
		Point notesPoint = View.centerString(dispNotes, new Rectangle(topLeftX + 12, topLeftY + 10 + 165, boxWidth + width - 24, 45), editMetric);
		...
		g.setFont(GUI.bodyFont);
		if(eventIndex == -1) {
			Point deletePoint = View.centerString(eventAbandonHint, new Rectangle(topLeftX + 12, topLeftY + 10 + 275 + 20 - 2, boxWidth + width - 24, 45), bodyMetric);
			g.drawString(eventAbandonHint, deletePoint.x, deletePoint.y);
		}else {
			Point deletePoint = View.centerString(eventDeleteHint, new Rectangle(topLeftX + 12, topLeftY + 10 + 275 + 20 - 2, boxWidth + width - 24, 45), bodyMetric);
	...
	/*
	 * Selected field:
	 * 	-1 is nothing is selected, should be set when user clicks on window but not on a field
	 * 	0 is title field is selected.
	 * 	1 is time field was clicked (and hence should show the time menu)
	 * 	2 is that the user clicked the start time field
	 * 	3 is that the user clicked the end time field
	 * 	4 is that the user clicked the note field
	 *  5 is that the user clicked the location field.
	 */
